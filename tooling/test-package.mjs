...
assert.deepEqual(
  pack.files.filter((file) => /LICEN[CS]E/i.test(file.path)),
  [],
  'this package grants no licence and must carry no licence file',
);
assert(
...
assert.equal(
  installed.license,
  'UNLICENSED',
  'the installed package must state that it grants no licence',
);
